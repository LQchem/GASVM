...
def remove_short_bond_length(geom):
...
def outlier_check(PES_E_list, GAN_E_list):
	# 检测异常值
...
if __name__=="__main__":
	...
	copy from A13, to match F66, only train E, not including D
	I don't remember what imrpovement is done from A13 other than the above. So use A13 in future
	some of the code in A13 is optimized, while not in this one
	'''
...
#			print(bigX.shape)
			pred_ED = generator_pes.forward(bigX)
	...
	#calc rmse of E and D
	a = np.array(E_error_list)
	rmse_E = np.sqrt(np.mean((a-np.mean(a))**2))

	print("rmse between generated Energy and PES energy", rmse_E)

	#plot the GA-svm data and the PES data
	#plot_ED(PES_E_list,GAN_E_list,ylabel='Energy (a.u.)',title='Energy.png')
	#plot(PES_D_list,GAN_D_list,ylabel='Norm of dipole moment (a.u.)',title='dipole.png')


	trunc_PES,trunc_GAN = outlier_check(PES_E_list, GAN_E_list)
...
#	PES_test2(norm_gen_data,NN_pes)
